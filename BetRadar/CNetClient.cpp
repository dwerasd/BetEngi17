...
namespace net
{
	...
	C_NET_CLIENT::C_NET_CLIENT(inet::C_IOCP_CALLBACK* _pCallback, LPCSTR _pAddress, WORD _nPort)
			...
			try
			{
				...
				else
				{
					if (INVALID_SOCKET != nSocket)
					{
							...
							do
							{
								...
								if (((_MAX_PACKET_SIZE_ - _ALIGNMENT_) - (WORD)pNetPacketBundle->nBytesTransferred) >= nCopySize)
								{
									...
									if (pNetPacketBundle->nBytesPacket == pNetPacketBundle->nBytesTransferred)
									{
										...
										{
											pCallback->ReceivePacket(pNetPacketBundle);
										}
						...
						{
							디뷰("네트워크 접속 끊김 >>> %d = ::recv()", nReciveSize);
							DSAFE_DELETE(pNetPacketBundle);
							Disconnect();
						}
					}
